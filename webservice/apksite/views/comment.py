...
from apksite.templatetags.apksite_comment_tags import comment_star_for, comment_star_thread
...
def comment_list(request, template='apksite/comment/comment_list.html'):
    initial = initial_validation(request, 'comment')
...
def comment_form(request, template='apksite/comment/comment_form.html'):
    target_object = initial_data(request.GET)
    if target_object and target_object.get('id'):
