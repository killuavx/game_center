...
class md5(object):
    @staticmethod
    def new(s):
        s = s.encode(errors='ignore') if hasattr(s, 'encode') else s
        return hashlib.md5(s)


base64.encodestring = lambda s: base64.encodebytes(s.encode(errors='ignore'))
...
class REST(object):
    ...
    def setSubAccount(self, SubAccountSid, SubAccountToken, VoIPAccount,
                      VoIPPassword):
    ...
    def callBack(self, fromPhone, to, customerSerNum, fromSerNum, promptTone):
        ...
        if self.BodyType == 'json':
            body = '''{"from": "%s", "to": "%s","customerSerNum": "%s","fromSerNum": "%s","promptTone": "%s"}''' % (
                fromPhone, to, customerSerNum, fromSerNum, promptTone)
        req.add_data(body)
        data = ''
        try:
    ...
    def landingCall(self, to, mediaName, mediaTxt, displayNum, playTimes,
        ...
        if self.BodyType == 'json':
            body = '''{"to": "%s", "mediaName": "%s","mediaTxt": "%s","appId": "%s","displayNum": "%s","playTimes": "%s","respUrl": "%s"}''' % (
    ...
    def voiceVerify(self, verifyCode, playTimes, to, displayNum, respUrl):
        ...
        if self.BodyType == 'json':
            # if this model is Json ..then do next code
            body = '''{"appId": "%s", "verifyCode": "%s","playTimes": "%s","to": "%s","respUrl": "%s","displayNum": "%s"}''' % (
                self.AppId, verifyCode, playTimes, to, respUrl, displayNum)
        req.add_data(body)
        data = ''
        try:
    ...
    def _make_signature(self, batch):
        signature = self.AccountSid + self.AccountToken + batch
        sig = md5.new(signature).hexdigest().upper()
        return sig

    def _make_auth(self, batch):
        src = self.AccountSid + ":" + batch
        return base64.encodestring(src).strip()

    def _make_request_url(self, func, sig):
        return "https://%(host)s:%(port)s/%(sdkversion)s/Accounts/%(account)s/%(func)s?sig=%(sig)s" % \
               dict(host=self.ServerIP ,
                    port=self.ServerPort,
                    sdkversion=self.SoftVersion,
    ...
    # @param datas 可选参数    内容数据
    # @param tempId 必选参数    模板Id
    def sendTemplateSMS2(self, to, datas, tempId):
        self.accAuth()
        nowdate = datetime.datetime.now()
        ...
        #生成auth
        auth = self._make_auth(batch)
        url = self._make_request_url(func="/SMS/TemplateSMS", sig=sig)
        req = urllib.request.Request(url)
