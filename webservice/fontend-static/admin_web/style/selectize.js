		...
		if (this.settings.render && typeof this.settings.render[templateName] === 'function') {
			...
			switch (templateName) {
				...
				case 'item':
					...
					break;
				case 'option_create':
					html = '<div class="create">创建 <strong>' + htmlEntities(data.input) + '</strong>&hellip;</div>';
					break;
			}
		}
	...
	*/
	...
	Selectize.registerPlugin('remove_button', function(options) {
		...
		this.settings.render.item = function(data) {
			...
			return '<div class="item">' + label + ' <a href="javascript:void(0)" class="remove" tabindex="-1" title="删除">&times;</a></div>';
		};
