...
(function($) {
	...
	var LG = 'linear-gradient(top, #e53333, #d12525)', CSS = '<style type="text/css">' +
		...
		'.wrap_out{z-index:998;left:-9999px; padding:8px; background:url(http://static.ccplay.com.cn/static/common/img/fix-b.png); border-radius:8px;-webkit-border-radius:8px;-moz-border-radius:8px; position:absolute}' +
		'.wrap_in{overflow: hidden; background:#f0f0f0;font-family:Microsoft Yahei}' +
		'.wrap_bar{border-bottom:1px solid #d12525;background:#e53333; background:-moz-'+ LG +';background:-o-'+ LG +';background:-webkit-'+ LG +';background:'+ LG +';}' +
		'.wrap_title{line-height:40px;padding-left:15px;color:#ffffff;margin:0;font-weight:bold;font-size:16px}' +
		'.wrap_close{position:relative}' +
		'.wrap_close a{width:40px;height:40px;text-align:center;margin-top:-40px;color:#fff;font:bold 1.5em/37px Tahoma;text-decoration:none;cursor:pointer;position:absolute;right:0}' +
		'.wrap_close a:hover{text-decoration:none!important;color:#333!important}' +
		'.wrap_body{background:white;font-size:14px}' +
		'.wrap_body p{padding:20px}' +
		'.submit_bg{background:#f7f7f7;border-top:1px solid #ccccd1;padding:5px;text-align:right}' +
		'.submit_btn, .cancel_btn{display:inline-block;padding:1px 12px 3px;border:1px solid;cursor:pointer;overflow:visible;font-family:Microsoft Yahei;font-size:14px}' +
		'.submit_btn{background:#e53333;border-color:#e53333 #d12525 #d12525 #e53333;color:#fff}' +
		'.submit_btn:hover{text-decoration:none;color:#fff}' +
		'.zxxbtn{text-align:right;background:#f0f0f0;padding:5px}' +
		'.cancel_btn{background:#c8c8c8;border-color:#c8c8c8 #999999 #999999 #c8c8c8;color:#333}' +
	'</style>';
	$("head").append(CSS);	  
	...
	$.extend($.zxxbox, {
		...
		ask: function(message, sureCall, cancelCall, options) {
			var element = $('<div class="wrap_remind"><p>'+message+'</p><div class="submit_bg"><button id="zxxSureBtn" class="submit_btn">确认</button>&nbsp;&nbsp;<button id="zxxCancelBtn" class="cancel_btn">取消</button></div></div>');
			$.zxxbox(element, options);
			//回调方法
		...
		remind: function(message, callback, options) {
			var element = $('<div class="wrap_remind"><p>'+message+'</p><div class="submit_bg"><button id="zxxSubmitBtn" class="submit_btn">确认</button</div></div>');
			$.zxxbox(element, options);
